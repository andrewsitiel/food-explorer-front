import { PaymentMadeScreens } from "../../components/PaymentMadeScreens";
...
export function Payment({ orderStatus }) {
  ...
  const PaymentOptions = isPixButtonSelected ? <img src={QR} alt="QR Code" /> : <CreditScreen />;
  const PaymentScreen = orderStatus ? PaymentMadeScreens(orderStatus) : PaymentOptions;
  ...
  return (
    ...
    >
      <button
        onClick={() => changeSelectedButton(true, false)}
      ...
      <button
        onClick={() => changeSelectedButton(false, true)}
        disabled={orderStatus ? true : false}>
        <HiOutlineCreditCard />
        Crédito
      </button>
